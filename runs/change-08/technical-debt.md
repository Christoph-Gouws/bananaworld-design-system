...
| **The three blocking advisories** | pre-existing on `main` and not created by this change. Escalated to the owner, **ANSWERED — option A**, and now **EXECUTED**: `next` → 15.5.25, `sharp` → 0.35.4, nothing added to `ignoreGhsas`, audit **0 blocking** (`known-issues.md` §A, decisions **D-9 / D-11**). Never debt at any point — filing it as debt would have implied someone chose to live with two unauthenticated RCEs, and the owner chose the opposite. ⚠ One *new* non-blocking follow-up did come out of it: 4 of the 6 standing `ignoreGhsas` entries are now inert and want retiring in a standalone housekeeping change — recorded in `known-issues.md` §A, deliberately not done here |
