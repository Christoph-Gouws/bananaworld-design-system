...
| | |
|---|---|
| `pnpm typecheck` | **clean** — re-run again in the CI-fix round on the bumped `next`, still clean |
| `pnpm test` — **baseline, measured BEFORE any edit** | **314 passed / 17 files** |
| `pnpm test` — final | **363 passed / 17 files** — re-run again in the CI-fix round on the bumped `next`, **363/363 still green** |
| New specs | **+49** |
...
| Dependency audit | ✅ **0 BLOCKING — see §7b.** Was 3, pre-existing on `main` and not caused by this change. **Owner decision A executed** in the CI-fix round: `next` → **15.5.25**, `sharp` → **0.35.4**, **nothing added to `ignoreGhsas`**. The walk was validated by reproducing CI's own pre-fix numbers before its post-fix number was believed |
| `pnpm install --frozen-lockfile` | ✅ **passes** — lock and manifest agree (CI's install step) |
...
## 7. Dependency audit — ✅ RESOLVED IN THE CI-FIX ROUND (was: the one thing stopping this change)

> **Read §7b first for the current state.** §7 and §7a below are the first two rounds' findings, kept
> because they are how the failure was diagnosed. As of the third round the audit measures
> **0 blocking**; `next` is at 15.5.25 and `sharp` at 0.35.4.
...
### 7b. THIRD ROUND — the fix landed, and the audit is green (2026-09-09, CI-fix session)

CI refused PR #22 on the `Dependency Audit` job alone. Its published failure line was:

```
16 vulnerabilities found
Severity: 7 moderate | 7 high (6 ignored) | 2 critical      -> exit 1
```

**The probe was validated against that line before it was trusted.** Re-run on the **pre-fix** tree it
returned `BLOCKING 3 | ignored 6 | moderate 7` — the same three GHSAs CI named, i.e. 1 non-ignored
high + 2 critical. A measurement that reproduces the failing job's own numbers is what licenses the
post-fix number:
...
**What was changed to get there** — two `pnpm.overrides` entries, re-resolved with
`pnpm install --lockfile-only`:

```
"next@<15.5.24":  "^15.5.24"     -> next  15.5.25
"sharp@<0.35.4":  "^0.35.4"      -> sharp 0.35.4
```

🔴 **`sharp` needed its own entry — the earlier claim that it would follow from the `next` bump was
wrong, and only running it revealed that.** next@15.5.25 widens its optional sharp range to
`^0.34.3 || ^0.35.4`, which `sharp@0.34.5` still satisfies, so the first re-resolution left sharp
where it was and `GHSA-rgj7-g3m4-5g8c` stayed red.

**Re-run green on the bumped tree**, all four, this session:

| Gate | Result |
|---|---|
| `pnpm install --frozen-lockfile` | ✅ lock and manifest agree — this is CI's install step |
| `pnpm typecheck` (`tsc --noEmit`) | ✅ clean — the peer bump moves no type |
| `pnpm test` | ✅ **363 passed / 363, 17 files** — unchanged from the first round |
| Audit closure walk | ✅ **0 blocking** |

**The lockfile was still never hand-authored** — the first round's refusal stands. It is generated by
pnpm; only the constraint it resolves against was edited.

⚠ The probe (`audit-probe.mjs`) was a throwaway at the repo root and **was deleted before the
commit.** It is not part of the change.
...
| `pnpm update` | still permission-gated. **No longer blocking:** `pnpm install --lockfile-only` **is** permitted, and decision **A** was executed through it plus two `pnpm.overrides` entries — §7b, D-11. The lockfile was never hand-authored |
...
| `pnpm audit:deps` | the script **was added this round** (byte-identical to CI's job), but the `pnpm audit` subcommand it wraps is still permission-gated, so it was **not** run under that name. The audit was measured by the closure walk in §7b instead, validated against CI's own numbers |
